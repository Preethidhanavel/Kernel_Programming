...
static DEFINE_RWLOCK(dev_rwlock); 
 
//Dynamic method to initialize the read write spinlock
//rwlock_t dev_rwlock;
 
unsigned long dev_global_variable = 0;
dev_t dev = 0;
static struct class *dev_class;
static struct cdev dev_cdev;
static struct task_struct *dev_thread1;
static struct task_struct *dev_thread2; 
...
int thread_function1(void *pv)
{
    while(!kthread_should_stop()) {  
        write_lock(&dev_rwlock);
        dev_global_variable++;
        write_unlock(&dev_rwlock);
        msleep(1000);
    }
    return 0;
}
...
{
    while(!kthread_should_stop()) {
        read_lock(&dev_rwlock);
        pr_info("In Thread 2 : Read value %lu\n", dev_global_variable);
        read_unlock(&dev_rwlock);
        msleep(1000);
    }
    return 0;
}
...
{
        kthread_stop(dev_thread1);
        kthread_stop(dev_thread2);
...
MODULE_DESCRIPTION("read and write spinlock");
