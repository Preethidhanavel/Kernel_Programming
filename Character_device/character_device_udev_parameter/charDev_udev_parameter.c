...
// Module parameters for device and class names
static char *device_name = "autodev";
static char *class_name  = "myclass";

module_param(device_name, charp, 0660);
MODULE_PARM_DESC(device_name, "Name of the device (appears in /dev/)");
module_param(class_name, charp, 0660);
MODULE_PARM_DESC(class_name, "Device class name (appears in /sys/class/)");
...
static int __init autodev_init(void)
{
    ...
    // 4. Create device node (/dev/<device_name>)
    ...
    printk(KERN_INFO "[%s] Device created: /dev/%s (class: %s)\n",
           device_name, device_name, class_name);
    return 0;
}
...
MODULE_DESCRIPTION("Char device with configurable /dev node and class name");
